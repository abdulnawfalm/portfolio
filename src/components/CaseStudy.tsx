...
/* ------------------------------------------------------------------
   The panel itself. Split out so it only ever mounts when a project is
   open - useScroll({ container }) needs its element to exist on the very
   first render, and inside the old `{project && ...}` block it didn't.
   ------------------------------------------------------------------ */
...
export default function CaseStudy({
  ...
  return (
    <AnimatePresence mode="wait">
      {project && (
        /* key on slug: switching projects remounts the panel, which resets
           the scroll position and re-measures the parallax for free. */
        <CaseStudyPanel
          key={project.slug}
          project={project}
          onClose={onClose}
